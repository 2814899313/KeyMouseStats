...
使用内置 image_gen 生成。六页独立场景及透明图标图集，资源嵌入 EXE。未指定或核实底层模型型号。
...
文件：minecraft-icons.png

Minecraft inventory icon atlas. EXACTLY 4 columns and 4 rows, 16 distinct icons evenly spaced in equal square cells. Genuine transparent alpha background, no grid lines, no written text, no UI labels, no painted checkerboard. Each icon fully contained with 12 percent transparent margins. Minecraft authentic voxel block geometry and crisp pixel textures, polished isometric 3D items, consistent warm studio lighting, green grass, oak brown, stone gray, diamond cyan, gold amber. Row1: grass block; minecart on short rail; Minecraft golden clock; crafting table. Row2: wooden chest; enchanting table with floating book; blocky gray computer mouse with cyan wheel; redstone repeater. Row3: compass; green potion bottle; stack of books; redstone dust with glowing signal. Row4: golden experience star trophy; lantern; large diamond gem; emerald gem. Clean readable chunky silhouettes, detailed game inventory renders. Square 1536x1536.
