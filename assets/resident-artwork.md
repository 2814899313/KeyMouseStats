...
通过内置 image_gen 生图工具生成六张独立主角背景和一套透明道具图集。未指定或核实底层模型型号。文件嵌入 EXE；渲染时按需读取并缓存。

总览：里昂；趋势：吉尔；时段：克里斯；键盘：克莱尔；应用：艾达；洞察：瑞贝卡。
...
文件：resident-icons.png

A premium Resident Evil inspired survival game inventory icon sprite sheet, EXACTLY 4 columns and 4 rows, 16 separate equal square cells on genuinely transparent alpha background. No words, no letters, no grid, no checkerboard painted into image. Each icon centered, fully inside its cell with transparent padding, distinct readable silhouette. Photorealistic detailed 3D prop renders, worn gunmetal, ivory paper, muted crimson accents, amber glass and subtle teal instrument lights. Dramatic coherent studio lighting, realistic material texture, not vector or line icons. Row1: antique mansion key with ornate crest; vintage tactical radio; round analog wristwatch; black vintage typewriter. Row2: folded city map with red pin; microscope with amber sample vial; rugged computer mouse; silver mechanical gear wheel. Row3: compass and route notebook; first aid spray can and green herb; linked file folders; portable heartbeat monitor. Row4: ornate gold police achievement medal; warm safe-room lantern; detailed silver and gold star-shaped police badge; red and white segmented umbrella medallion. Square 1536x1536. No weapons, no gore. Every object isolated and separate, consistent scale, no overlapping cells.
